"""
Test per self_learning.py — copre in particolare is_decisive_win() (definita
in trade_manager.py e importata qui, non più una copia locale _is_win): la
logica di classificazione win/loss corretta il 2026-09-03 dopo che tre file
diversi (dashboard.py, self_learning.py, gold_bot.py) avevano iniziato a
divergere silenziosamente su come contare un WIN_BE con TP1 già raggiunto.
self_learning.py aveva comunque tenuto una quarta copia locale (_is_win),
mai consolidata insieme alle altre tre — rimossa in favore dell'import.
"""
...
class TestIsWin(unittest.TestCase):
    def test_win_tp1_tp2_tp3_are_wins(self):
        for result in ("WIN_TP1", "WIN_TP2", "WIN_TP3"):
            with self.subTest(result=result):
                self.assertTrue(sl.is_decisive_win({"result": result, "tp1_hit": 0}))

    def test_loss_is_never_a_win(self):
        self.assertFalse(sl.is_decisive_win({"result": "LOSS", "tp1_hit": 1}))
    ...
    def test_win_be_with_tp1_hit_counts_as_win(self):
        ...
        self.assertTrue(sl.is_decisive_win({"result": "WIN_BE", "tp1_hit": 1}))

    def test_win_be_without_tp1_hit_is_neutral_not_a_win(self):
        self.assertFalse(sl.is_decisive_win({"result": "WIN_BE", "tp1_hit": 0}))

    def test_cancelled_is_not_a_win(self):
        self.assertFalse(sl.is_decisive_win({"result": "CANCELLED", "tp1_hit": 0}))
        ...
        self.assertTrue(sl.is_decisive_win({"result": "CANCELLED", "tp1_hit": 1}))

    def test_missing_fields_do_not_raise(self):
        self.assertFalse(sl.is_decisive_win({}))
